<template>
  <div>
    <v-layout row justify-center>
      <v-dialog v-model="receptionSellEdit" persistent max-width="900" v-if="receptionCustomerSell">
        <v-card>
            ...
            <v-card-text class="mt-4">
                ...
                >
                  <v-container>
                          ...
                          > 
                            ...
                            <template v-slot:item.no="{ item }">
                              <v-checkbox
                                ...
                                :value="true"
                                hide-details
                                :disabled="item.bikeStatus == null || receptionCustomerSell.status == 'completed'"
                                class="mt-0"
                        ...
                        <v-row class="mt-4">
                          <v-col cols="12">
                              <p class="title text-right font-weight-bold"> Total: <span class="orange--text"> 2.000$ </span> </p>
                          </v-col>
                        </v-row>
                  </v-container>
                </v-form>
    ...
    <transaction v-if="receptionSellTransaction" :getReceptionCustomerSell.sync="getReceptionCustomerSell"></transaction>
...
<script>
// component 
import VehicleTestComponent from '@/components/technical/test/VehicleTest.vue'
...
export default {
    ...
    methods: {
      ...
      save(){
        if (this.$refs.form.validate()) {
            console.log(this.receptionCustomerSell)
        }
          else{
            console.log("Fail")
        }
      ...
      async transaction(){
        ...
        var vehicleTestSelected = this.vehicleTests.filter(item => item.selected === true)
    ...
    computed: {
      ...
      rangePrice:{
        get: function(){
           return  this.$helper.defaultNumber(this.vehicleTest.minPrice) + ' - ' + this.$helper.defaultNumber(this.vehicleTest.maxPrice)
        },
        set: function(newVal){
