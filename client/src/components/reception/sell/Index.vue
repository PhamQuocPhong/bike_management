...
<div id="reception">
	<v-container>
	      ...
	      <v-card class="pa-4">
	        ...
	        >
			    ...
			    <div class="mb-6">
				    ...
				    <v-row>
				    	<v-col cols="6" :key="index" v-for="(item, index) in vehicleTestArray">
				    		<v-hover
	    ...
	    <add-vehicle v-if="receptionSellAddVehicle"
	    ...
	    create: async function(){
	        ...
	        if (this.$refs.form.validate()) {
	        	...
	        	if(checkEmptyVehicle || this.vehicleTestArray.length <= 0){
	        		toastr.warning('<p>Please add vehilce info!</p>', 'Warning', {timeOut: 500})
	        		return
	        	}
	        	...
	        	var dataStore = {
					...
					{	...obj, 
						vehicleTypeId: obj.vehicleType.id != null ? obj.vehicleType.id   : null,
				...
				if(res.status === 400)
				 ...
				 {
					 toastr.error('<p>Error!</p>', 'Not yet added vehicles', {timeOut: 500})
				 }
	 			else if(res.status === 200){
	 				await ReceptionCustomerSell.insert({
	 					data: res.data.data
	 				})
	 				toastr.success('<p>Create success!</p>', 'Success', {timeOut: 500})
	          	...
	          	toastr.warning('<p>Please fill out this form!</p>', 'Warning', {timeOut: 500})
	        }
	    },
