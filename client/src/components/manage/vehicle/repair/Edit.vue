<template>
    <v-layout row justify-center>
      <v-dialog v-model="editDialog" persistent max-width="900" v-if="getVehicleRepair">
                  ...
                  <v-container>
                        <v-row v-if="getVehicleRepair">
                          ...
                          <v-col cols="6">
                           ...
                           <v-col cols="6">
                            ...
                            >
                              ...
                              <v-date-picker v-model="getVehicleRepair.startDate" no-title @input="menu1 = false"></v-date-picker>
                            </v-menu>
                          </v-col>
                          ...
                          <v-col cols="6">
                            ...
                            >
                              ...
                              <v-date-picker v-model="getVehicleRepair.endDate" no-title @input="menu1 = false"></v-date-picker>
                            </v-menu>
                          </v-col>
                        ...
                        <v-row >
                          <v-col 
                            ...
                            >
                            <v-subheader class="font-weight-bold">Planning fixing list</v-subheader>
                            <v-spacer></v-spacer>
                            <v-btn 
                            color="success" 
                            small
                                ...
                                <template v-for="(item, index) in getTechnicalRepairs">
                                  <v-list-item :key="item.id">
                                    <template v-slot:default="{ active }">
                                      <v-list-item-content>
                                        <v-list-item-title 
                                        class="font-weight-bold" v-text="item.accessory"></v-list-item-title>
                                        <v-list-item-subtitle 
                                        class="red--text text--lighten-1 font-italic pt-2 pb-2"
                                        v-text="item.employee.fullName"></v-list-item-subtitle>
        ...
        <add-fix-detail v-if="dialogAddFixDetail" :dialogAddFixDetail.sync="dialogAddFixDetail" :vehicleRepairId.sync="getVehicleRepair.id"></add-fix-detail>
...
export default {
    ...
    async created(){
      ...
      const res = await TechnicalRepair.api().fetchByVehicleRepairId(this.vehicleRepair.id)
    ...
    computed: {
      getTechnicalRepairs(){
        return TechnicalRepair.query().with('employee').where('vehicleRepairId', this.getVehicleRepair.id).get()
      },

       formatStartDate () {
        return HelperCommon.formatDate(this.getVehicleRepair.startDate)
      },
      formatEndDate () {
        return HelperCommon.formatDate(this.getVehicleRepair.endDate)
      },
