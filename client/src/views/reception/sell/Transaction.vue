          ...
          <v-container>
            ...
            <v-card>
              <div class="col col-12">
                <p class="sub-title text-right font-weight-bold">
                  Sub total:
                  <span class="orange--text">
                    {{ subTotalPrice | toCurrency }}
                  </span>
                </p>
                <p class="sub-titl text-right font-weight-bold">
                  Tax price:
                  <span class="orange--text">
                    {{ taxPrice | toCurrency }}
                  </span>
...
export default {
  ...
  created() {
    var data = this.vehicleTests
      .map(obj => ({
        ...obj,
        averagePrice: Math.ceil(
          (this.$helper.defaultNumber(obj.minPrice) +
            this.$helper.defaultNumber(obj.maxPrice)) /
            2
        ),
        reducePrice: 0,
  ...
  methods: {
    ...
    calcPrice(val, id) {
      this.transactionDetailBuy
        .filter(obj => obj.id === id)
        .map(
          obj => (
            (obj.reducePrice = this.$helper.defaultNumber(val)),
            (obj.totalPrice =
              obj.averagePrice - this.$helper.defaultNumber(val))
          )
        );
      this.calcSubTotalPrice();
      this.calcTaxPrice();
    ...
    calcTotalPrice() {
      this.totalPrice =
        parseFloat(this.subTotalPrice) + parseFloat(this.taxPrice);
    },
