                      ...
                      >
                        <template v-slot:activator="{ on }">
                          <v-text-field
                            ...
                            persistent-hint
                            prepend-icon="event"
                            @blur="getTechnicalRepair.startDate = $helper.parseDate(dateFormatted)"
                            v-on="on"
                          ></v-text-field>
                        </template>
                        <v-date-picker v-model="getTechnicalRepair.startDate" no-title></v-date-picker>
                      </v-menu>
                  </v-col>
                      ...
                      >
                        <template v-slot:activator="{ on }">
                          <v-text-field
                            ...
                            persistent-hint
                            prepend-icon="event"
                            @blur="getTechnicalRepair.endDate = $helper.parseDate(dateFormatted)"
                            v-on="on"
                          ></v-text-field>
                        </template>
                        <v-date-picker v-model="getTechnicalRepair.endDate" no-title></v-date-picker>
                      </v-menu>
                  </v-col>
  ...
  export default {
    ...
    data(){
      return {
        ...
        dateFormatted: this.$helper.formatDate(new Date().toISOString().substr(0, 10)),
      }
    },
