	    			...
	    			>
	    				<v-simple-table :class="{mobile: isMobile}">
						    <template v-slot:default v-if="!isMobile">
						      ...
						      <tbody>
						        <tr v-for="item in vehicleTests" :key="item.id" v-if="loadData">
						        	<td>
						    ...
						    <template v-slot:default v-else>
						    	<tr v-for="item, index in vehicleTests" :key="item.id" v-if="loadData">
						        	<td>
						        		<ul class="flex-content">
						        			<li class="flex-item" data-label="No."> 
						        				{{$helper.indexColumn(item, vehicleTests)}}
						        			</li>
						        			<li class="flex-item" data-label="Accessory">{{item.bikeCode}}</li>
...
export default{
	...
	methods: {
	    ...
	    async retrieveData(){
			...
			progress.start()
			const res = await TechnicalTest.api().fetchPaging(this.currentPage, this.itemsPerPage)
