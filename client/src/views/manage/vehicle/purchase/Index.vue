		    			...
		    			>
			    		  	<v-simple-table :class="{mobile: isMobile}">
							    ...
							    <template v-slot:default v-else>
							    	<tr v-for="item, index in vehiclePurchases" :key="item.id" v-if="loadData">
							        	<td>
									    	...
									    	<li class="flex-item" data-label="Image"><v-img  height="50" width="100" contain :src="item.image"></v-img></li>
									    	<li class="flex-item" data-label="Code">{{item.code}}</li>
									    	<li class="flex-item" data-label="Registration plate">{{item.registrationPlate}}</li>
									    	<li class="flex-item" data-label="Status">
										    	<v-chip :color="$helper.colorStatusBike(item.status)"  class="justify-center status-bike" small dark> 
									    			{{item.status}} 
									    		</v-chip>
									    	</li>
									    	<li class="flex-item" data-label="Price">{{item.price | toCurrency}}</li>
