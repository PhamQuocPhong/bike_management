            ...
            <v-row>
              ...
              <v-col cols="6">
                <div class="d-flex">
                  <label>Date:</label>
                  <p class="pl-4 font-weight-bold">
                    {{ $helper.formatDate(transactionData.date) }}
                  </p>
                </div>
              </v-col>
            </v-row>
...
export default {
  ...
  watch: {
    orderBuyEdit(val) {
      if (!val) Modal.dispatch("orderBuyEdit", { option: "hide" });
    }
  },
  ...
  computed: {
    transactionDetails() {
      return this.transactionData.transactionDetailBuys
        .map(obj => ({
          ...obj,
