	...
	>
		...
		<v-simple-table :class="{mobile: isMobile}">
		    ...
		    <template v-slot:default v-else>
		    	<tr v-for="item, index in transactionSells" :key="item.id" v-if="loadData">
		        	<td>
		        		<ul class="flex-content">
				        	<li class="flex-item" data-label="Action">
				        		{{$helper.indexColumn(item, transactionSells)}}
				        	</li>
					    	<li class="flex-item" data-label="Action">{{item.employee.fullName}}</li>
					    	<li class="flex-item" data-label="Action">{{item.customer.name}}</li>
					    	<li class="flex-item" data-label="Action">{{item.totalItem}}</li>
					    	<li class="flex-item" data-label="Action">{{item.subTotalPrice}}</li>
					    	<li class="flex-item" data-label="Action">{{item.taxPrice}}</li>
					    	<li class="flex-item" data-label="Action">{{item.totalPrice}}</li>
...
export default{
	...
	methods: {
	    ...
	    async retrieveData(){
	    	...
	    	this.loadData = false
	    	setTimeout(async () => {
	    		const res = await Transaction.api().fetchSellPaging(this.currentPage, this.itemsPerPage)
